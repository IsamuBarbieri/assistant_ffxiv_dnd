# 09_ASSETS — Verbatim assets (for the assistant)
Version a1.00 (host-agnostic) | Source: FFXIV x D&D 5e Homebrew - assistant operating manual

## SCHEMA NOTES
- FILE CONTRACT: this file holds ONLY assets the assistant REPRODUCES CHARACTER BY CHARACTER. An asset is COPIED; a rule is APPLIED. The rules that decide WHEN and WITH WHAT DATA an asset is emitted live in 06 — this file never states behaviour.
- Retrieve this file ONLY when a command asks for the asset by name. Nothing here is consulted while writing a beat.
- CODE FENCES ARE PERMITTED IN THIS FILE and only here: an asset is source text, so the fence is what preserves it. 06 §A1's fence ban governs OUTPUT PROSE, not this file.
- ALIGNMENT: `combat_tracker.html` in the repo is the browser-openable copy. THIS FILE IS AUTHORITATIVE; the direction is always HTML -> 09, never the reverse. On any touch, diff the two ignoring the demo data block.

## §Z1 — COMBAT TRACKER TEMPLATE (was 06 §A24.1)
Emit this VERBATIM, changing ONLY the `encountersData` payload and the subtitle. What the controls do, the `statblocks` panel, the per-assistant scope and the fill procedure: 06 §A24.
