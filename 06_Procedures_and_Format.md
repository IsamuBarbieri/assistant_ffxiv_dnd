...
Version v5.01 (host-agnostic) | Source: FFXIV x D&D 5e Homebrew - assistant operating manual
...
**THIS BLOCK IS FOR A CHECK, NOT FOR AN ENIGMA (binding, and it says WHERE the other shape lives — a ban with no alternative just leaves you on the template you can see):** the tiers above are a ladder of RESULTS ('what it gets'), which is right for `/prova` and WRONG for a puzzle, where it makes the die the solution. **An interlude/enigma uses the FIVE-BLOCK SHAPE in §B12** (`Da leggere ai PG` → `Enigma per i PG` → `Soluzione (GM)` → `Indizi` → `Fallimento`), whose tiers are a ladder of INFORMATION and where no tier may accomplish the objective. Same for a mystery: §E5.
