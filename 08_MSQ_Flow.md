...
- CUT (binding): the quest's only content is the materia-system demo, which this homebrew does not implement (no other file references it). NEVER played, NEVER summarised, NEVER the target of an 'apre': 'Dressed to Deceive' closes straight into **Lord of the Inferno**. Entry kept only as a canonical trace.
